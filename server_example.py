...
save_filepath = './online/convert/Nightop-Wen+200001.wav'
...
def get_from_client():
    print (save_filepath, file=sys.stderr)
    return send_file(save_filepath, mimetype='audio/wav')
...
def post_data_from_router():
    ...
    inputFilePath = receive_dir + '/'+ f.filename
    name, audioType = f.filename.split('.')
    outputFileName = '2%05d.wav' % int(name)
    denoiseOutputFilePath = receive_dir + '/'+ 'denoise_' + outputFileName
    ...
    cmd = 'ffmpeg -y -i ' + inputFilePath + ' -acodec pcm_s16le -f s16le -ac 1 -ar 48000 input.pcm'
    os.system(cmd)
    ...
    cmd = 'ffmpeg -f s16le -y -ar 24k -ac 2 -i output.pcm ' + denoiseOutputFilePath
    os.system(cmd)
    ...
    # speaker prediction
    source = get_speaker(denoiseOutputFilePath)
    print (source,file=sys.stderr)
    conversion(denoiseOutputFilePath, source, target)
    return 'POST_OK'
...
def conversion(filepath, source, target):
    ...
    speaker = source
    _, __, dname = filepath.rsplit('/', maxsplit=2)
    _, name = dname.split('_')
    print (name)
